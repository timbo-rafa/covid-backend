...
export class CountryCovidMapper {
  mapEntitiesTo2dTableDto(countryEntities: CountryWithAllCovidDataEntity[]): CountryCovidTableDto[] {
    const dtos = countryEntities.map((countryEntity) => {
      ...
      for (const vaccination of covidVaccinations) {
        ...
        const { date, newVaccinations, peopleFullyVaccinated, peopleVaccinated, totalBoosters, totalVaccinations } = vaccination;
        if (!countryTableByDateDictionary[timestamp]) {
          countryTableByDateDictionary[timestamp] = this.initializeCountryDto({ id, isoCode, name, date });
        }
